...
module Make (W : Writer) = struct
  ...
  let word t x = W.le_uint64 t.w x
  ...
  let event ?(payload = []) ~ty ?(args = []) t ~name ~thread ~category ~ts =
    ...
    let words =
      2 + List.length payload + Thread_ref.size thread + String_ref.words name
      + String_ref.words category + Args.words args
    in
    record t ~ty:4 ~words
      ~data:
        (i64 ty
    ...
    Args.write t args;
    List.iter (word t) payload

  let instant_event = event ~ty:0 ~payload:[]
  let counter_event ~id = event ~ty:1 ~payload:[ id ]
  let duration_begin = event ~ty:2 ~payload:[]
  let duration_end = event ~ty:3 ~payload:[]
  ...
  let kernel_object ?(args = []) t ~name ty id =
    ...
    record t ~ty:7 ~words
      ~data:
        (i64 ty ||| (i64 (String_ref.encode name) <<< 8) ||| (i64 argc <<< 24));
    word t id;
    Args.write t args
